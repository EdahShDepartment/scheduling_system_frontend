...
watch(() => employeeStore.employees, (newEmployees) => {
  newEmployees.forEach(emp => {
    if (flexibleHoursStore.bigFrame.min_leave_default[emp.name] === undefined) {
      flexibleHoursStore.setMinLeaveDefault(emp.name, 8)
    }
  })
...
const addRule = () => {
  if (selectedEmployee.value) {
    ...
    Object.assign(newRule, newRuleDefaults)
  }
}
        ...
        <div v-if="selectedEmployee">
          <div class="mb-4">
            ...
            <table class="min-w-full leading-normal text-sm"
              v-if="flexibleHoursStore.getSmallFrameRules(selectedEmployee).length">
              ...
              <tbody>
                <tr v-for="rule in flexibleHoursStore.getSmallFrameRules(selectedEmployee)" :key="rule.id">
          ...
          <form @submit.prevent="addRule" class="p-4 border-t bg-gray-50 rounded-b-lg">
            ...
            <div class="grid grid-cols-2 md:grid-cols-5 gap-3 items-end">
              <div class="col-span-2 md:col-span-1">
                ...
                <input type="number" v-model.number="newRule.start_day" required class="w-full p-1 border rounded-md">
              </div>
              <div class="col-span-2 md:col-span-1">
                <label class="block text-xs text-gray-600">結束日</label>
                <input type="number" v-model.number="newRule.end_day" required class="w-full p-1 border rounded-md">
              </div>
              <div class="col-span-1">
                <label class="block text-xs text-gray-600">ZZ</label>
                <input type="number" v-model.number="newRule.ZZ" required class="w-full p-1 border rounded-md">
              </div>
              <div class="col-span-1">
                <label class="block text-xs text-gray-600">YY</label>
                <input type="number" v-model.number="newRule.YY" required class="w-full p-1 border rounded-md">
              </div>
              <button type="submit"
          ...
          <div v-for="employee in employeeStore.employees" :key="`overview-${employee.id}`"
            ...
            <div class="p-3">
              <table class="min-w-full leading-normal text-xs"
                v-if="flexibleHoursStore.getSmallFrameRules(employee.name).length">
                <thead class="bg-gray-50">
                  <tr>
                ...
                <tbody>
                  <tr v-for="rule in flexibleHoursStore.getSmallFrameRules(employee.name)" :key="rule.id"
                    class="text-gray-700">
